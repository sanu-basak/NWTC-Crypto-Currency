...
#if defined(NDEBUG)
# error "NWTCcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xea0720b1c1e7ce2e6b0273ff5aae7920a0c18930a0f712cda46c3c6324fa859b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // NWTCcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "NWTCcoin Signed Message:\n";
...
{
    // NWTCcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // NWTCcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // NWTCcoin: 5 mintues
static const int64 nTargetSpacing = 1 * 30; // NWTCcoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // NWTCcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // NWTCcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xea0720b1c1e7ce2e6b0273ff5aae7920a0c18930a0f712cda46c3c6324fa859b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is NWTCCoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1505472085;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1439948;
        ...
        assert(block.hashMerkleRoot == uint256("0x9e7a3fd5d8c89c912d358e42ba3ca1617c616ce4cb3717e92ece268bc3c3e2f9"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // NWTCcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NWTCcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NWTCcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NWTCcoinMiner(CWallet *pwallet)
{
    printf("NWTCcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("nwtc-miner");
    ...
    try { loop {
        ...
        printf("Running NWTCcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NWTCcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NWTCcoinMiner, pwallet));
}
